...
class Engine:
    ...
    def generate_batched(
        self,
        prompts: list[list[int]],  # List of different token sequences
        max_tokens=None,
        temperature=1.0,
        top_k=None,
        seed=42,
        ...
        """
        Generates completions for a batch of different prompts in parallel.
        Uses left-padding to align prompts.
        """
        ...
        # 1) Prepare Batch with Left Padding
        # We left-pad so the last token of every prompt aligns at the same step
        max_len = max(len(p) for p in prompts)
        padded_ids = []
        for p in prompts:
            padding = [pad_token_id] * (max_len - len(p))
            padded_ids.append(padding + p)
        
        # Shape: (B, T)
        ids = torch.tensor(padded_ids, dtype=torch.long, device=device)

        # 2) Batched Prefill
        # We process all prompts in parallel. No cache expansion needed.
        m = self.model.config
        kv_model_kwargs = {
            "num_heads": m.n_kv_head,
            "head_dim": m.n_embd // m.n_head,
        ...
        # Initialize KV Cache for the full batch immediately
        kv_length_hint = (max_len + max_tokens) if max_tokens else self.model.config.sequence_len
        kv_cache = KVCache(
        ...
        # Forward pass on the prompts
        logits = self.model.forward(ids, kv_cache=kv_cache)
        logits = logits[:, -1, :] # Take the logits of the last token
        next_ids = sample_next_token(logits, rng, temperature, top_k)
        sampled_tokens = next_ids[:, 0].tolist()

        # 3) Initialize RowStates
        # Each row gets its own original (unpadded) tokens
        row_states = [RowState(p.copy()) for p in prompts]

        # 4) Main Generation Loop
        num_generated = 0
        
        # Prepare ids for the first decode step
        ids = torch.tensor(sampled_tokens, dtype=torch.long, device=device).unsqueeze(1) # (B, 1)
        ...
        while True:
            ...
            # Process tokens from previous step (either prefill or decode)
            token_column = []
            token_masks = []
            
            # Update states and prepare next input tokens
            for i, state in enumerate(row_states):
                # ... [Existing State Logic is identical] ...
                # Use sampled_tokens[i] which aligns with this row
                ...
                # If completed, we just feed the last token (or pad) to keep shapes valid, 
                # but we stop updating the state.
                token_column.append(next_token)

                # Check completion
                if not state.completed and (next_token == assistant_end or next_token == bos):
                    state.completed = True
            ...
            # We must run forward even for completed rows to keep KV cache synchronized 
            # (unless we implement complex active-row masking)
