...
depths=(4 6 8)           # model depth
n_heads_list=(4 6 8)     # number of heads (KV‑heads = heads)
max_seq_lens=(1024)      # max sequence length
device_batch_sizes=(64)  # batch size per device

# These are scalars, so they don't generate a tag
sample_every=100         # how often to sample
eval_every=100           # how often to evaluate

n_streams_list=(4)       # number of streams (GPUs)
aspect_ratios=(32 64)    # aspect ratio
            ...
            for device_batch_size in "${device_batch_sizes[@]}"; do
                for n_streams in "${n_streams_list[@]}"; do
                    for aspect_ratio in "${aspect_ratios[@]}"; do
                        ...
                        model_tag="${tag#_}"
                        job_name="depth${depth}_heads${n_heads}_${model_tag}"
                        ...
                        python -m scripts.base_train \
                            ...
                            --model_tag="$job_name" \
                            --num_iterations=3000 >"$log_file" 2>&1
