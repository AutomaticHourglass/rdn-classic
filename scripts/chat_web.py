...
parser.add_argument('-t', '--temperature', type=float, default=0.1, help='Default temperature for generation')
...
async def chat_completions(request: ChatRequest):
    ...
    try:
        ...
        response_tokens = []
        async def stream_and_release(start, cnt):
            try:
                async for chunk in generate_stream(
                    ...
                    if "token" in chunk_data:
                        if chunk_data["token"] == '"' or chunk_data["token"] == '?':
                            if start:
                                cnt += 1
                            else:
                                continue
