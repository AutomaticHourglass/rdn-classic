...
from rdn.tokenizer import get_tokenizer, RustBPETokenizer
# from rdn.dataset import parquets_iter_batched
...
def str_prob():
    ...
    return f"{prob['prompt']}{prob['json']}"

train_docs = [str_prob() for _ in range(100)]
# train_docs = next(tokenizing_distributed_data_loader_with_state(128, 2048, split="train"))
train_text = "\n".join(train_docs)
# val_docs = next(parquets_iter_batched(split="val"))
# train_docs = next(tokenizing_distributed_data_loader_with_state(128, 2048, split="val"))
val_docs = [str_prob() for _ in range(100)]
val_text = "\n".join(val_docs)
